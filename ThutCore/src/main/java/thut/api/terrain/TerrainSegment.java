...
public class TerrainSegment
{
            ...
            else
            {
                ...
                if (world.villageCollectionObj != null)
                {
                    Village village = world.villageCollectionObj
                            .getNearestVillage(new BlockPos(MathHelper.floor_double(v.x), MathHelper.floor_double(v.y),
    ...
    public void refresh(World world)
    {
        ...
        if (sky)
        {
            for (int i = 0; i < 16; i++)
            {
                sky = getCentre().add(0, -1 * i, 0).getBlock(world) == null
                        || getCentre().add(0, -1 * i, 0).getBlock(world).getMaterial() == Material.air;
                if (!sky) break;
            }
        }
        if (sky)
