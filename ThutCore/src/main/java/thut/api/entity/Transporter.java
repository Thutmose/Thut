...
public class Transporter {
	...
	public static class TelDestination
	{
		...
		public TelDestination(World _dim, AxisAlignedBB srcBox, double _x, double _y, double _z, int tileX, int tileY, int tileZ) {
			...
			this.yOff = tileY;
			this.zOff = tileZ;
			this.loc = Vector3.getNewVectorFromPool().set(x,y,z);
		}
		...
		{
			this(FMLCommonHandler.instance().getMinecraftServerInstance().worldServerForDimension(dim), loc.getAABB(), loc.x, loc.y, loc.z, loc.intX(), loc.intY(), loc.intZ());
		}
	...
	//	if ( diffDestination )
		{
			...
			else
			{
				...
				if ( (entity.addedToChunk) && (oldWorld.getChunkProvider().chunkExists( entX, entZ )) )
				{
					...
					oldWorld.getChunkFromChunkCoords( entX, entZ ).setChunkModified();//.isModified = true;
				}
...
//						h.field_146062_d += link.zOff;
	...
	public static Entity teleportEntity(Entity entity, Vector3 t2, int dimension, boolean destBlocked) {
		...
		if (!destBlocked) {
			if (entity.dimension == dimension)
				newEntity = teleportWithinDimension(entity, t2, destBlocked);
			else {
	...
	static Entity teleportWithinDimension(Entity entity, Vector3 p, boolean destBlocked) {
		if (entity instanceof EntityPlayerMP)
			return teleportPlayerWithinDimension((EntityPlayerMP)entity, p);
		else
			return teleportEntityToWorld(entity, p, (WorldServer)entity.worldObj, destBlocked);
	}
	...
	static Entity teleportToOtherDimension(Entity entity, Vector3 p, int dimension, boolean destBlocked) {
		if (entity instanceof EntityPlayerMP) {
			...
			return teleportEntityToDimension(entity, p, dimension, destBlocked);
	}
	...
	static Entity teleportEntityToWorld(Entity oldEntity, Vector3 p, WorldServer newWorld, boolean destBlocked) {
		Vector3 v = Vector3.getNewVectorFromPool().setToVelocity(oldEntity);
