...
//			index--;
//			if (ret != null) return ret.setBounds(minX, minY, minZ, maxX, maxY, maxZ);
//		}
...
//		}
	...
	public Vector3 doTileCollision(IBlockAccess world, Entity e, Vector3 offset, Vector3 diffs)
	{
		...
		if(e.riddenByEntity != null)
			maxY += (e.riddenByEntity.height + e.getMountedYOffset());
...
//        System.out.println(yTop+" "+e.posY+" "+e.stepHeight+" "+yShift);
	...
	{
		if (!(e instanceof IMultibox)) return false;

		Vector3 diffs1 = Vector3.getNewVectorFromPool().set(diffs);
		Vector3 off = Vector3.getNewVectorFromPool().set(e);
		diffs1.set(doTileCollision(world, e, off, diffs));
