package thut.lootcrates.config;
...
import thut.lootcrates.LootCrates;
...
public class Config
{
    public static abstract class ConfigData implements IConfigHolder
    {
        ...
        {
            LootCrates.LOGGER.info("Loaded {} config file {}", this.MODID, configEvent.getConfig().getFileName());
            this.read(configEvent.getConfig());
        }
        ...
        {
            LootCrates.LOGGER.info("Reading {}", config.getFileName());
            ...
            for (final Field f : values.keySet())
                try
                {
                    ...
                    LootCrates.LOGGER.info("Set {} to {}", f.getName(), o);
                    f.set(this, o);
                    changed = true;
                }
                catch (final Exception e)
                {
                    LootCrates.LOGGER.error("Error updating config value for " + f, e);
                }
            return changed;
        }
        ...
        private boolean write(final ModConfig config, final Map<Field, ConfigValue<?>> values)
        {
            ...
            for (final Field f : values.keySet())
                ...
                {
                    LootCrates.LOGGER.error("Error saving config value for " + f, e);
                }
            return ret;
        }
    }
