...
public class TileEntityKiln extends TileEntityMultiCore
{
	...
	public boolean checkIfProperlyFormed()
	{
		...
		for (int k = -1; k < 5; k++)
		{
			for (int i = 0; i < 7; i++)
			{
				for (int j = -3; j < 4; j++)
				{
					...
					if (k == 2 && i == 3 && j == 0)
					{
						hole.set(xCoord + (axis ? i * dir : j), zCoord + (axis ? j : i * dir), yCoord + k);
						continue;
					}
					...
					// worldObj.setBlock(x, y, z, partBlocks);
		...
		{
			n = 0;
			axis = (facing.equals(ForgeDirection.EAST) || facing.equals(ForgeDirection.WEST));
			dir = (facing.equals(ForgeDirection.WEST) || facing.equals(ForgeDirection.NORTH)) ? -1 : 1;
		...
		for (int k = 0; k < 4; k++)
			for (int i = 0; i < 7; i++)
				for (int j = -3; j < 4; j++)
				{
					...
					Vector3 loc = Vector3.getNewVectorFromPool().set(x, y, z);
		...
		for (int k = 0; k < 4; k++)
			for (int i = 0; i < 7; i++)
				for (int j = -3; j < 4; j++)
				{
					...
					{
						loc.freeVectorFromPool();
						if (k == 0) loc.setBlock(worldObj, fire);
					}
					// else
	...
	public ItemStack decrStackSize(int slot, int count)
	{
		if (this.furnaceItems[slot] != null)
		{
			...
			if (furnaceItems[slot].stackSize <= 0) furnaceItems[slot] = null;
	...
	public boolean isUseableByPlayer(EntityPlayer entityPlayer)
	{
		return worldObj.getTileEntity(xCoord, yCoord, zCoord) != this ? false
				: entityPlayer.getDistanceSq((double) xCoord + 0.5, (double) yCoord + 0.5,
						(double) zCoord + 0.5) <= 64.0;
	}
	...
	public void readFromNBT(NBTTagCompound tagCompound)
	{
		...
		for (int i = 0; i < itemsTag.tagCount(); i++)
		{
			...
			if (slot >= 0 && slot < furnaceItems.length) furnaceItems[slot] = ItemStack.loadItemStackFromNBT(slotTag);
		}
	...
	{
		if (currentItemBurnTime == 0) currentItemBurnTime = getCookTime();
	...
	public boolean cookable(ItemStack stack)
	{
		...
		if (type == 0) for (ItemStack cook : ThutItems.cookable)
		{
			// System.out.println(cook.itemID + " " + stack.itemID);
			if (cook.isItemEqual(stack)) { return true; }
		}
		if (type == 1) if (stack.getItem() == Item.getItemFromBlock(stone)) return true;
	...
	private boolean canSmelt(int stack)
	{
		...
		{
			// System.out.println("not cookable"+stack+"
			// "+cookable(furnaceItems[stack])+" "+furnaceItems[stack]);
			return false;
		}
		else
		{
			ItemStack itemStack = null;
			if (type == 0) itemStack = ThutItems.lime.copy();
			if (type == 1) itemStack = ThutItems.dust.copy();

			if (itemStack == null) return false;

			if (furnaceItems[stack + 9] == null) return true;
			if (!furnaceItems[stack + 9].isItemEqual(itemStack)) return false;
	...
	public void smeltItem(int stack)
	{
		if (canSmelt(stack))
		{
			...
			if (furnaceItems[stack + 9] == null) furnaceItems[stack + 9] = itemStack.copy();
			...
			if (furnaceItems[stack].stackSize <= 0) furnaceItems[stack] = null;
		}
	}
	...
	{
		return slot > 8 && !TileEntityFurnace.isItemFuel(itemstack) ? false
	...
	{
		if (type == 0) return "thutconcrete.container.limekiln";
