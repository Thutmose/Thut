...
public abstract class BlockGas extends BlockFluidFinite
{
	...
	public BlockGas(int id, Fluid fluid) {
		super(id, fluid, Material.air);
	...
	public void onEntityCollidedWithBlock(World worldObj, int x, int y, int z, Entity entity) 
	{
		...
		if(up.getBlock(worldObj).isBlockNormalCube(worldObj, x, y, z)&&here.getBlockMetadata(worldObj)>3)
	...
	public int countSides(World worldObj, int x, int y, int z, int id)
	{
		...
		{
			if(worldObj.getBlockId(x+side.offsetX, y+side.offsetY, z+side.offsetZ)==id)
				ret++;
		}
		return ret;
	}
