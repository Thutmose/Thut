...
public class Vector3 
{
	  ...
	  public  static Vector3 findNextSolidBlock(World worldObj,Vector3 source,Vector3 direction, double range){
			...
			for(double i=0;i<range;i+=0.0625)
			{
			  ...
			  if(!(Int(xtest)==Int(xprev)&&Int(ytest)==Int(yprev)&&Int(ztest)==Int(zprev)))
			  {
				  ...
				  boolean clear = block==null||block.isAirBlock(worldObj, Int(xtest), Int(ytest), Int(ztest))||(new Vector3(xtest, ytest, ztest)).isFluid(worldObj);
    ...
    public boolean isFluid(World world)
    {
    	...
    	{
    		fluidCheck = fluids.get(s).getBlockID()==id&&fluids.get(s).getViscosity()!=Integer.MAX_VALUE;
    		if(fluidCheck)
    			return true;
    	}
    	
    	ret = fluidCheck || id == Block.waterMoving.blockID ||  id == Block.lavaMoving.blockID;
	...
	public Block getBlock(IBlockAccess worldObj, ForgeDirection side) {
		return Block.blocksList[worldObj.getBlockId(intX()+side.offsetX, intY()+side.offsetY, intZ()+side.offsetZ)];
	}
	...
	public int getBlockId(IBlockAccess worldObj, ForgeDirection side) {
		return worldObj.getBlockId(intX()+side.offsetX, intY()+side.offsetY, intZ()+side.offsetZ);
	}
	...
	public boolean isAir(IBlockAccess worldObj) 
	{
		if(worldObj instanceof World)
		{	
			return getBlock(worldObj)==null||getBlockMaterial(worldObj)==null||getBlock(worldObj).isAirBlock((World) worldObj, intX(), intY(), intZ())||getBlockMaterial(worldObj)==Material.air;//||worldObj.isAirBlock(intX(), intY(), intZ())
		}
	...
	{
		return worldObj.getBlockTileEntity(intX()+side.offsetX, intY()+side.offsetY, intZ()+side.offsetZ);
	}
	public void scheduleUpdate(World worldObj)
	{
		if(getBlock(worldObj)!=null)
		worldObj.scheduleBlockUpdate(intX(), intY(), intZ(), getBlockId(worldObj), getBlock(worldObj).tickRate(worldObj));
	}
	public void breakBlock(World worldObj, boolean drop)
	{
		if(getBlock(worldObj)!=null)
			worldObj.destroyBlock(intX(), intY(), intZ(), drop);
	}
	public boolean isBlockOnSideSolid(IBlockAccess worldObj, ForgeDirection side)
	{
		boolean ret = worldObj.isBlockSolidOnSide(intX()+side.offsetX, intY()+side.offsetY, intZ()+side.offsetZ, side.getOpposite(), false);
		return ret;
	}
}