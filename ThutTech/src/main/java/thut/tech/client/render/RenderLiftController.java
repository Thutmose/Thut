...
public class RenderLiftController<T extends TileEntity> extends TileEntityRenderer<T>
{
    ...
    public void drawEditOverlay(final TileEntityLiftAccess monitor, final Direction side)
    {
        // Call button toggle
        // Draw the white background
        GL11.glPushMatrix();
        Color colour = new Color(255, 255, 255, 255);
        this.drawOverLay(monitor, 1, colour, side, false);
        ...
        this.drawOverLay(monitor, 2, colour, side, false);
    ...
    public void drawOverLay(final TileEntityLiftAccess monitor, int floor, final Color colour, final Direction side,
            final boolean wide)
    {
        ...
        if (floor > 0 && floor < 17)
        {
            ...
            float dz = -0.001f;
            if (monitor.getWorld() instanceof IBlockEntityWorld<?>) dz = -0.005f;
            floor -= 1;
            final double x = (double) (3 - floor & 3) / (double) 4, y = ((double) 3 - (floor >> 2)) / 4;
    ...
    {
        final TileEntityLiftAccess monitor = (TileEntityLiftAccess) tileentity;
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            else if (monitor.floorDisplay[dir.ordinal()])
            {
                ...
                Color colour = new Color(255, 255, 255, 255);
                this.drawOverLay(monitor, 1, colour, dir, true);
                GL11.glPopMatrix();
            ...
            else
            {
                ...
                for (int j = monitor.getSidePage(dir) * 16; j < 16 + monitor.getSidePage(dir) * 16; j++)
                    this.drawOverLay(monitor, j + 1, colour, dir, false);
                if (monitor.lift != null)
                {
                    colour = new Color(0, 255, 0, a);
                    this.drawOverLay(monitor, monitor.floor, colour, dir, false);
                    colour = new Color(255, 255, 0, a);
                    this.drawOverLay(monitor, monitor.lift.getDestinationFloor(), colour, dir, false);
                    colour = new Color(0, 128, 255, a);
                    this.drawOverLay(monitor, monitor.lift.getCurrentFloor(), colour, dir, false);
                    for (int j = monitor.getSidePage(dir) * 16; j < 16 + monitor.getSidePage(dir) * 16; j++)
                    {
                        colour = new Color(10, 10, 10, 128);
                        if (!monitor.lift.hasFloors[j]) this.drawOverLay(monitor, j + 1, colour, dir, false);
                    }
                }
            }
            GL11.glPopMatrix();
