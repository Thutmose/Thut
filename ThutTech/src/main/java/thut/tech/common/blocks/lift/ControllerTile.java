...
// SimpleComponent
{
    ...
    public void buttonPress(final int button, final boolean callPanel)
    {
        ...
        else if (button != 0 && button <= this.lift.floors.length && this.lift != null
                && this.lift.hasFloors[button - 1])
        {
    ...
    public boolean doButtonClick(final LivingEntity clicker, final Direction side, final float hitX, final float hitY,
            final float hitZ)
    {
        if (this.liftID != null && !this.liftID.equals(this.empty)
                && this.lift != EntityLift.getLiftFromUUID(this.liftID, this.world))
            this.lift = EntityLift.getLiftFromUUID(this.liftID, this.world);
    ...
    public void tick()
    {
        ...
        if (this.lift != null && !this.world.isRemote)
        {
            ...
            if (callPanel && !old && !this.lift.getCalled() && !check)
                if (this.world.isBlockPowered(this.getPos())) this.lift.call(this.floor);
        ...
        if (this.lift != null && this.floor > 0)
        {
            ...
            // floor.
            if ((int) Math.round(this.lift.posY) == this.lift.floors[this.floor - 1])
        ...
        if (this.lift == null && this.liftID != null)
        {
            ...
            if (this.liftID != null && !this.liftID.equals(this.empty)
                    && (this.lift == null || !this.lift.isAlive() || tempLift != this.lift))
            {
                this.lift = tempLift;
