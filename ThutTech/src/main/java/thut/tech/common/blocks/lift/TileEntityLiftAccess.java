...
public class TileEntityLiftAccess extends TileEntity implements ITickableTileEntity// ,
// SimpleComponent
{
    public static final TileEntityType<? extends TileEntity> TYPE = TileEntityType.Builder.create(
            TileEntityLiftAccess::new, TechCore.LIFTCONTROLLER).build(null);
    ...
    {
        super(TileEntityLiftAccess.TYPE);
    }

    public TileEntityLiftAccess(final TileEntityType<?> tileEntityTypeIn)
    {
    ...
    {
        final int state = 1;
        // System.out.println(Arrays.toString(this.sides));
    ...
    public void tick()
    {
        ...
        if (this.lift != null && !this.world.isRemote)
        {
            ...
            boolean old = state.get(BlockLift.CURRENT);
            ...
            if (check != old)
            {
                state = state.with(BlockLift.CURRENT, check);
            ...
            {
                old = state.get(BlockLift.CALLED);
                check = this.lift.getDestinationFloor() == this.floor;
                if (check != old)
                {
                    state = state.with(BlockLift.CALLED, check);
                    this.world.setBlockState(this.getPos(), state);
                }
            }
        ...
        if (this.lift == null && this.tick++ % 50 == 0) for (final Direction side : Direction.values())
        {
            ...
            {
                final TileEntityLiftAccess te = (TileEntityLiftAccess) t;
                if (te.lift != null)
                {
                    this.lift = te.lift;
                    this.floor = te.floor;
