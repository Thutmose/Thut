...
public class BiomeList {
	...
	{
		if(biomes.containsKey(rgb))
			return biomes.get(rgb).mineCraftBiome;
		return 0;
	}
	
	public static int getBiomeFromValues(int biome, int temperature, int drainage, int rainfall, int evil, Region region)
	{
		...
		if(region!=null)
		{
			if(region.type==RegionType.GLACIER && BiomeDictionary.isBiomeOfType(b, Type.PLAINS))
			{
				boolean cold = BiomeDictionary.isBiomeOfType(b, Type.COLD) || BiomeDictionary.isBiomeOfType(b, Type.SNOWY);
				if(!cold)
				{
	...
	private static BiomeGenBase getMatch(BiomeGenBase toMatch, Type type)
	{
		...
		for(int j = 0; j<biomeArray.size(); j++)
		{
			...
			if(b!=toMatch && b!=null)
			{
				...
				for(Type t: existing)
				{
					if(!BiomeDictionary.isBiomeOfType(b, t))
						continue biomes;
				}
				return b;
			}
